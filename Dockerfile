...
FROM cychen2022/jfs-ubuntu2204-build:latest as jfs_base
...
RUN ln -s /tmp/z3-4.8.15-install/lib/libz3.so /usr/lib/libz3.so.4.8
...
RUN sudo apt-get update && sudo apt-get install -y openssh-server gdb gdbserver rsync vim git cmake \
    ...
    g++-multilib python3-tabulate
RUN sudo pip install lit

RUN echo "root:klee" | sudo chpasswd
RUN sudo sed -r -i 's/#?\s*PermitRootLogin prohibit-password/PermitRootLogin yes/' /etc/ssh/sshd_config

RUN mkdir -p /home/klee/colossus-re
RUN sudo chsh root -s /bin/bash
RUN sudo ln -s /home/user/colossus-re /root/colossus-re
WORKDIR /home/klee/workdir


FROM klee_base as intermediate
